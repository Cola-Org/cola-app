...
App.plusDownload = function (url, fileName, callback) {
	var options = {
		method: "GET",
	...
	var task = plus.downloader.createDownload(App.prop("serviceUrlPrefix") + "file/" + url, options, function (task, status) {
...
App.goLogin = function (nextPath, callback) {
	...
	if (this.rootApp) {
		...
		var path = loginPath, realNextPath = nextPath || (cola.getCurrentRoutePath() + location.search);
...
// Determine "mobile.contextPath"
var i = window.location.pathname.lastIndexOf("/");
App.setProp("mobile.contextPath", window.location.pathname.substring(0, i));
